...
const UPDATES: UpdateEntry[] = [
  {
    version: "0.2.2",
    storageKey: "zelixvary:update-seen:v0.2.2",
    trTitle: "🌍 Yeni Güncelleme! İngilizce Dili Geldi",
    enTitle: "🌍 New Update! English Language Is Here",
    trSubtitle: "🚀 0.2.2 sürümüyle neler geldi:",
    enSubtitle: "🚀 Here's what's new in version 0.2.2:",
    trItems: [
      "🌍 **İngilizce dil desteği geliştirildi!** — Sitenin **tamamı** artık 🇹🇷 Türkçe **ve** 🇬🇧 İngilizce! Tek tıkla dil değiştirin, tüm yazılar anında seçtiğiniz dile geçsin",
      "🔄 **Dil değiştirici eklendi** — Sağ üstteki 🌍 globe ikonundan dili seçin: Ana sayfa, mod seçim ekranı ve stüdyo başlığında mevcut",
      "📝 **150+ metin çevrildi** — Landing page, giriş & kayıt, mod seçimi, editör, AI sohbet, ayarlar, dosya gezgini, Python terminali ve projeler — hepsi iki dilde",
      "🤖 **AI mesajları artık iki dilde** — Hoşgeldin mesajı, hızlı aksiyonlar, dosya seçim ekranı ve AI yönlendirmesi seçtiğiniz dilde çalışır",
      "💾 **Dil tercihiniz kaydedilir** — Seçtiğiniz dil tarayıcınızda saklanır; sayfayı yenilediğinizde ve tekrar girdiğinizde korunur",
      "🔤 **HTML dil etiketi güncellenir** — `lang` otomatik olarak `tr` / `en` arasında değişir (erişilebilirlik & SEO için)",
      "🐛 **Dil menüsü hatası düzeltildi** — Ana sayfada dil seçilemiyordu (menü tıklamaları yutuluyordu), artık sorunsuz çalışıyor",
    ],
    enItems: [
      "🌍 **English language support developed!** — The **entire site** is now 🇹🇷 Turkish **and** 🇬🇧 English! Switch with one click and every text instantly changes",
      "🔄 **Language switcher added** — Pick your language via the 🌍 globe icon in the top right: available on the landing page, mode selection screen and studio header",
      "📝 **150+ strings translated** — Landing page, login & registration, mode selection, editor, AI chat, settings, file explorer, Python terminal and projects — all in both languages",
      "🤖 **AI messages now bilingual** — Welcome message, quick actions, file picker and AI routing all work in your chosen language",
      "💾 **Your language preference is saved** — Your choice is stored in your browser and kept when you reload or come back",
      "🔤 **HTML lang attribute updates automatically** — `lang` switches between `tr` / `en` automatically (for accessibility & SEO)",
      "🐛 **Language menu bug fixed** — Language selection wasn't working on the home page (clicks were being swallowed); now it works seamlessly",
    ],
  },
  {
    version: "0.2.1",
...
/* ── En son sürüm (v0.2.3 — Yeni yapay zeka sistemi) ───────── */
const LATEST: UpdateEntry = {
  version: "0.2.3",
  storageKey: "zelixvary:update-seen:v0.2.3",
  trTitle: "🤖 Yeni Güncelleme! 3 Yeni Yapay Zeka",
  enTitle: "🤖 New Update! 3 New AIs",
  trSubtitle: "🚀 0.2.3 sürümüyle neler geldi:",
  enSubtitle: "🚀 Here's what's new in version 0.2.3:",
  trItems: [
    "🤖 **3 yeni yapay zeka sağlayıcısı!** — ⚡ **Groq Cloud**, 🔶 **Cerebras Inference** ve 🌬️ **Mistral AI** artık destekleniyor — ultra hızlı ve ücretsiz modellerle kodla",
    "🎛️ **Yeni nesil API ayarları** — Daha geniş panel, **her satırda 2 model**, model **arama çubuğu** ve ⭐ **favorileme sistemi** — beğendiğin modeli yıldızla, tekrar arama",
    "🧩 **Yapay zeka seçim ekranı** — Local / Dosya modunu seçince artık yapay zeka seçim ekranı gelir; stüdyodaki ⚙️ Ayarlar'dan istediğin an değiştirirsin",
  ],
  enItems: [
    "🤖 **3 new AI providers!** — ⚡ **Groq Cloud**, 🔶 **Cerebras Inference** and 🌬️ **Mistral AI** are now supported — code with ultra-fast, free models",
    "🎛️ **Next-gen API settings** — Bigger panel, **2 models per row**, a model **search bar** and a ⭐ **favorites system** — star a model and find it instantly",
    "🧩 **AI selection screen** — Picking Local / File mode now shows an AI selection screen; change it anytime from ⚙️ Settings in the studio",
  ],
};
