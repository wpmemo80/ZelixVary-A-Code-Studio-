...
const AIChat = forwardRef<AIChatHandle, AIChatProps>(function AIChat(
  ...
  const determineProvider = useCallback(
    (prompt: string): { provider: Parameters<typeof chatCompletions>[0]["provider"]; model: string; reason: string } => {
      if (selection !== AUTO_ROUTER_ID) {
        ...
        // localStorage'dan kaydedilmiş modeli kontrol et
        let model = cfg.defaultModel;
        try {
          const saved = localStorage.getItem(`zelixvary:model:${cfg.id}`);
          if (saved && cfg.models.includes(saved)) model = saved;
        } catch { /* yut */ }
        return { provider: cfg.id, model, reason: "" };
      }
  ...
  function confirmPick() {
    ...
    const prompts: Record<ChatAction, string> = {
      debug: buildDebugPrompt(combined + (extra ? `\n\n// Ekstra İstek: ${extra}` : "")),
      refactor: buildRefactorPrompt(combined + (extra ? `\n\n// Ekstra İstek: ${extra}` : "")),
      ui: buildUiPrompt(extra || combined),
      chat: "",
    };
    const history = messages;
          ...
          <div className="w-full max-w-md rounded-2xl border border-zinc-700 bg-[#1c1c1f] p-5 shadow-2xl">
            ...
            <div className="mt-3">
              <label className="mb-1.5 block text-[11.5px] font-medium text-zinc-400">
                📝 Ekstra İstek <span className="text-zinc-600">(isteğe bağlı)</span>
              </label>
              <textarea
                value={extraRequest}
                ...
                placeholder={
                  pickTarget === "ui"
                    ? "Örn: Kubernetes temalı bir dashboard yap, sidebar olsun..."
                    : pickTarget === "refactor"
                      ? "Örn: Functional component'e çevir, TypeScript ekle..."
                      : "Örn: Bu hatayı düzelt, performansı artır..."
                }
                className="w-full resize-none rounded-lg border border-zinc-700 bg-[#101013] px-3 py-2 text-[12px] text-zinc-200 placeholder-zinc-600 outline-none transition focus:border-violet-500"
              />
              <p className="mt-1 text-[10px] text-zinc-600">
                Yapay zekaya ek bilgi vermek istersen buraya yaz. Seçili dosyalardaki kodla birlikte gönderilir.
              </p>
            </div>
