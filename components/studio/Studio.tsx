...
export default function Studio({ mode, onBackToSelection }: StudioProps) {
  ...
  const isProjectMode = mode === "file" && project !== null;

  // Local modda otomatik olarak tek dosya modunda çalış
  const isLocalMode = mode === "local";

  const activeCode = useMemo(() => {
    if (isLocalMode) return code;
    if (project) {
      if (currentFile) return project.files[currentFile] ?? "";
      return "";
    }
    return code;
  }, [isLocalMode, project, currentFile, code]);
  ...
  const handleEditorChange = useCallback(
    ...
    [isLocalMode, project, currentFile, setCode],
  );
  ...
  // File modunda otomatik olarak dosya seçiciyi açma — kullanıcı isterse manuel açsın
  // useEffect(() => {
  //   if (mode === "file" && !project) {
  //     const t = setTimeout(() => {
  //       openPickedFolder();
  //     }, 300);
  //     return () => clearTimeout(t);
    ...
    <div className="flex h-screen flex-col overflow-hidden bg-[#101013] text-zinc-200">
      ...
      <header className="flex h-11 shrink-0 items-center gap-2 border-b border-zinc-800 bg-[#1c1c1f] px-3">
        <button
          ...
          className="flex items-center gap-1.5 rounded-md border border-zinc-700 px-2 py-1 text-[11.5px] font-medium text-zinc-300 transition hover:border-violet-500/60 hover:text-violet-300"
        ...
        <span
          className={`rounded-full px-2 py-0.5 text-[10px] font-bold tracking-wider ${
            isLocalMode
              ? "border border-violet-500/50 bg-violet-500/15 text-violet-300"
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="flex min-w-0" style={{ width: `${split}%` }}>
          {/* Dosya modunda her zaman FileExplorer göster (proje olsun ya da olmasın) */}
          {mode === "file" && (
              ...
              <FileExplorer
                ...
                onCreateFile={(path, content) => {
                  if (project) {
                    ...
                    setProject(newProject);
                    savedSignatureRef.current = JSON.stringify(newProject.files);
                  }
                  setCurrentFile(path);
                }}
          ...
          {/* Local modda dosya gezgini gösterme */}
          {isLocalMode && isProjectMode && (
            <div className="w-52 shrink-0 border-r border-zinc-800">
              <FileExplorer
                files={project.files}
                currentFile={currentFile}
